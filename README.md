...
The ignored root `.env` holds `OPENAI_API_KEY`. Do not use a `VITE_` variable for the key. `OPENAI_DIAGNOSIS_MODEL` defaults to the account-tested `gpt-6-astra`; diagnosis uses **medium reasoning** and one API call. Priority processing is enabled for lower latency at a higher per-token price (`OPENAI_DIAGNOSIS_SERVICE_TIER=default` selects standard processing). Restart the API after environment or server changes.
...
The prompt separates component/assembly identity from failure diagnosis and provides curated, cited vehicle anatomy and troubleshooting notes (`shared/diagnosis-references.js`). References describe normal vehicles; the demo article’s damage assessment is never sent. Text-only requests explicitly contain zero image inputs, accept described damage as reported evidence, and ask symptom/test questions rather than demanding photographic proof. Limited evidence can still identify a useful assembly; only unusable evidence or a different vehicle blocks localization. Exact manual instructions retain their stricter evidence and edition gates. All diagnoses remain provisional. Only allowlisted source groups can be highlighted, and these are approximate assembly context, not photo-to-mesh registration. Current assets lack verified target-part geometry and tool paths: **repair animations remain unavailable**. The animated inspection separates the original model groups. Manual references are conditional and restricted to the verified edition/vehicle match; an unrelated static repair is never paired with the live assessment.
...
Validation: `npm test` includes modality, one-photo enforcement, cross-vehicle/manual mismatches, API failure, image bounds, limited-evidence assembly context, text-only provenance, pricing, and actual source-group checks. A stronger model is not an accuracy guarantee. Before production, evaluate a labeled, technician-reviewed damage set, including poor photos, ambiguous components and no-fault cases; measure wrong-part identification and abstention, then validate each manual-to-mesh binding separately.

Live checks on 2026-09-19: GPT-5.4 missed the displaced C8 damper in the wider demo image. GPT-6 Astra identified the rear shock mounting assembly in both photos in 9.6–10.3 seconds using medium reasoning, original image detail and the fast tier. These are smoke checks, not accuracy or latency guarantees. The earlier two-pass path is no longer used.

Run `npm run eval:diagnosis` explicitly for six paid API smoke cases: two public C8 photos, three synthetic text-only reports and a blank-image negative case. `-- --case c8-reported-mount` runs one case. Expected answers are evaluated locally and never included in API requests. The default JSON report is `/tmp/vehicle-diagnosis-evaluation.json` (`DIAGNOSIS_EVAL_OUTPUT` overrides it). The evaluator checks useful identification/localization, no invented photo evidence, no demands for photos, stop-use routing when appropriate and no unsupported repair animation. It does not establish general mechanical diagnostic accuracy.
