...
export default function VehicleViewer({vehicle=VEHICLES['yzf-2021']}) {
  ...
  return <div className="motion-lab assembly-lab">
    <header className="lab-header"><a className="lab-brand" href="/vehicles/yzf-2021"><span className="brand-symbol">◇</span><span>MOTION<span className="brand-light"> / LAB</span></span></a><div className="header-center"><span className="status-dot"/>VEHICLE INSPECTION STUDIO</div><BikeSwitcher value={vehicle.id}/></header>
    <main className="assembly-workspace">
      <section className="assembly-stage" aria-label="Vehicle viewer">
        ...
        <div className="model-heading"><div className="eyebrow">{vehicle.brand.toUpperCase()} <span>/</span> {vehicle.edition.toUpperCase()}</div><h1>{vehicle.name}<span>{tab==='repair'?'REPAIR / SOURCE GEOMETRY':'IMPORTED ASSEMBLY'}</span></h1><p>{tab==='repair'?'Inspect the same parts, down to each source mesh.':'Inspect the assembly. Understand the repair.'}</p></div>
