...
export default function RepairPanel({vehicle,ready,onVehicle,onPhase,onCaseStep,children}){
      ...
      <fieldset disabled={busy||loadingPhotos}>
        <div className="diagnosis-intake-grid">
          ...
          <div className="diagnosis-evidence-column">
            ...
            <small className="diagnosis-help">A description alone is fine. Avoid guessing the failed part; describe what you observed.</small>
  ...
  return <section className="diagnosis-flow diagnosis-results" ref={root} aria-label="API diagnosis result">
    ...
    <p className="diagnosis-vehicle-summary">{vehicle.brand} {vehicle.name} · {result.provenance.inputMode}</p>
    ...
    <section className="diagnosis-section"><h3>What the API observed</h3>{result.observations.length?result.observations.map((o,i)=><div className="diagnosis-observation" key={i}><span>{o.source==='image'?`PHOTO ${o.imageIndex+1}`:'YOUR REPORT'}</span><p>{o.text}</p></div>):<p>No usable observations yet.</p>}</section>
    ...
    <section className="diagnosis-section"><h3>What’s still unknown</h3><ul>{result.uncertainties.map(s=><li key={s}>{s}</li>)}</ul>{result.questions.length>0&&<><h4>Questions that would help</h4><ol>{result.questions.map(q=><li key={q}>{q}</li>)}</ol><button className="diagnosis-outline" onClick={edit}>Answer questions / replace photo ↗</button></>}</section>
    <section className="diagnosis-section"><div className="diagnosis-section-number">03 / LOCATION</div><h3>{result.location.label||'3D location not established'}</h3><p>{result.location.reason}</p><small>Assembly context only. Photo pixels are not registered to an exact 3D service part.</small>{region&&<><button className="diagnosis-outline" disabled={!ready} onClick={()=>{setPlaying(false);onCaseStep({groupId:region,spacing:0,mode:'studio'});}}>Focus this assembly in 3D ⊙</button><button className="diagnosis-outline" disabled={!ready} onClick={()=>{setTourStep(0);setPlaying(!playing);}}>{playing?'Pause inspection':'Play exploded inspection'} {playing?'Ⅱ':'▷'}</button>{playing&&<small role="status">{['Locate the source assembly','See its context in X-ray','Separate the original groups'][tourStep]}</small>}</>}
      <div className="diagnosis-animation-state"><strong>Repair animation unavailable</strong><p>{result.repair.animationReason}</p><small>Repairability: {result.repair.repairability}.</small></div>
    </section>
    <section className="diagnosis-section"><div className="diagnosis-section-number">04 / NEXT ACTION</div><h3>What to ask a repair shop</h3><p>{result.repair.requestText}</p>
